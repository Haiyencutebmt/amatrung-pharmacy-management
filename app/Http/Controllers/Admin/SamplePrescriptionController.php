...
class SamplePrescriptionController extends Controller
{
    ...
    {
        return redirect()->route('admin.treatment-templates.index', ['tab' => 'prescriptions']);
    }

    public function create()
    {
        $herbs = MedicinalHerb::orderBy('name')->get();
        return view('admin.sample_prescriptions.create', compact('herbs'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'suggested_condition' => 'nullable|string',
            'usage_instruction' => 'nullable|string',
            'preparation_type' => 'nullable|string|max:100',
            'default_packages' => 'nullable|integer|min:1',
        ...
        $sample = DB::transaction(function () use ($request) {
            $sample = SamplePrescription::create([
                'name' => $request->name,
                'suggested_condition' => $request->suggested_condition,
                'usage_instruction' => $request->usage_instruction,
                'preparation_type' => $request->preparation_type,
                'default_packages' => $request->default_packages,
                'notes' => $request->notes,
            ]);
            ...
            foreach ($request->items as $item) {
                SamplePrescriptionItem::create([
                    ...
                    'medicinal_herb_id' => $item['medicinal_herb_id'],
        ...
        session()->put('last_action', [
            'model' => SamplePrescription::class,
            'type' => 'create',
            'id' => $sample->id,
            'redirect_url' => route('admin.treatment-templates.index', ['tab' => 'prescriptions']),
        ]);

        return redirect()->route('admin.treatment-templates.index', ['tab' => 'prescriptions'])->with('success', 'Thêm bài thuốc mẫu thành công.');
    }

    public function edit(SamplePrescription $samplePrescription)
    {
        $samplePrescription->load('items.medicinalHerb');
        $herbs = MedicinalHerb::orderBy('name')->get();
        return view('admin.sample_prescriptions.edit', compact('samplePrescription', 'herbs'));
    }
    ...
    public function update(Request $request, SamplePrescription $samplePrescription)
    {
        ...
        $originalData = [
            'sample' => $samplePrescription->getOriginal(),
            'items' => $samplePrescription->items->toArray(),
        ];

        DB::transaction(function () use ($request, $samplePrescription) {
            $samplePrescription->update([
                'name' => $request->name,
                'suggested_condition' => $request->suggested_condition,
                'usage_instruction' => $request->usage_instruction,
                'preparation_type' => $request->preparation_type,
            ...
            $samplePrescription->items()->delete();

            foreach ($request->items as $item) {
                SamplePrescriptionItem::create([
                    'sample_prescription_id' => $samplePrescription->id,
                    'medicinal_herb_id' => $item['medicinal_herb_id'],
                    'quantity' => $item['quantity'],
                ]);
            }
        ...
        session()->put('last_action', [
            'model' => SamplePrescription::class,
            'type' => 'update',
            'id' => $samplePrescription->id,
            'original_data' => $originalData,
            'redirect_url' => route('admin.treatment-templates.index', ['tab' => 'prescriptions']),
        ]);

        return redirect()->route('admin.treatment-templates.index', ['tab' => 'prescriptions'])->with('success', 'Cập nhật bài thuốc mẫu thành công.');
    }

    public function show(SamplePrescription $samplePrescription)
    {
        $samplePrescription->load('items.medicinalHerb');
        return response()->json($samplePrescription);
    }

    public function destroy(SamplePrescription $samplePrescription)
    {
        $samplePrescription->load('items');
        $originalData = [
            'sample' => $samplePrescription->getOriginal(),
            'items' => $samplePrescription->items->toArray(),
        ];

        $samplePrescription->delete();
        ...
        return redirect()->route('admin.treatment-templates.index', ['tab' => 'prescriptions'])->with('success', 'Đã xóa bài thuốc mẫu.');
    }
    ...
    public function bulkDestroy(Request $request)
    {
        $request->validate([
            'ids' => 'required|array',
            'ids.*' => 'exists:sample_prescriptions,id',
        ]);

        $ids = $request->ids;
        $originalData = [];
        foreach ($ids as $id) {
            $sp = SamplePrescription::with('items')->find($id);
            if ($sp) {
                $originalData[] = [
                    'sample' => $sp->getOriginal(),
                    'items' => $sp->items->toArray(),
                ];
                $sp->delete();
            }
        }
        ...
        return redirect()->route('admin.treatment-templates.index', ['tab' => 'prescriptions'])->with('success', 'Đã xóa các bài thuốc mẫu đã chọn.');
    }
        ...
        foreach ($initialFormulas as $name => $data) {
            DB::transaction(function () use ($name, $data) {
                ...
                foreach ($data['herbs'] as $herbName => $qty) {
                    // Tìm hoặc tạo dược liệu trong kho
