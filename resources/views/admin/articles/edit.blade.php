...
@section('page-title', 'Sửa Bài Viết: ' . $article->title)
...
<div class="max-w-[1200px] mx-auto px-2 md:px-4 mt-2">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl md:text-3xl font-black text-slate-800 tracking-wide">Chỉnh Sửa Bài Viết</h1>
            <p class="text-sm text-slate-500 mt-1">Cập nhật thông tin y học cổ truyền, điều chỉnh hình ảnh minh họa cho bài viết.</p>
        </div>
    ...
    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
        ...
        <!-- Hidden field to handle image deletion -->
            ...
            <div class="lg:col-span-8 space-y-6">
                <div class="bg-white rounded-[2rem] p-6 md:p-8 shadow-[0_10px_30px_rgba(0,0,0,0.02)] border border-slate-100">
                    ...
                    <div class="form-group mb-6">
                        ...
                        <input type="text" name="title" id="title" class="w-full px-5 py-3.5 rounded-2xl border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all shadow-sm text-lg font-bold text-slate-800 placeholder-slate-300" value="{{ old('title', $article->title) }}" required placeholder="Nhập tiêu đề bài viết...">
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <div class="border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                            <textarea name="content" id="content" class="w-full outline-none p-4" rows="15">{{ old('content', $article->content) }}</textarea>
                        </div>
                        <p class="text-xs text-slate-400 mt-2 flex items-center gap-1.5">
                            ...
                            Mẹo: Chỉnh sửa nội dung và trình bày trực quan bằng bảng công cụ TinyMCE.
                        </p>
                        @error('content')
                            <p class="text-red-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                        @enderror
                ...
                <div class="bg-white rounded-[2rem] p-6 shadow-[0_10px_30px_rgba(0,0,0,0.02)] border border-slate-100">
                    ...
                    <div class="flex items-center gap-3 py-1">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer select-none">
                            <input type="checkbox" name="is_published" id="is_published" value="1" class="sr-only peer" {{ old('is_published', $article->is_published) ? 'checked' : '' }}>
                    ...
                    <div class="flex gap-3 mt-6 pt-4 border-t border-slate-100">
                        ...
                        <button type="submit" class="btn btn-primary w-1/2 rounded-xl py-2.5 font-bold shadow-md shadow-blue-500/10">Cập nhật</button>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[2rem] p-6 shadow-[0_10px_30px_rgba(0,0,0,0.02)] border border-slate-100">
                    ...
                    <div class="form-group">
                        ...
                        <div class="mb-2">
                            ...
                            <!-- Image preview container: visible if featured_image exists -->
                            <div id="image-preview-container" class="relative w-full h-48 rounded-2xl overflow-hidden shadow-inner border border-slate-100 {{ $article->featured_image ? '' : 'hidden' }} group">
                                <img id="image-preview" src="{{ $article->featured_image ? Storage::url($article->featured_image) : '#' }}" alt="Preview" class="w-full h-full object-cover">
...
<script>
  ...
  function previewFile(input) {
      ...
      if (file) {
          ...
          reader.onload = function(e) {
              ...
              document.getElementById('remove_featured_image').value = '0'; // reset delete flag
          }
          reader.readAsDataURL(file);
      }
  }
  ...
  function removePreviewImage() {
      ...
      document.getElementById('remove_featured_image').value = '1'; // set delete flag to 1
  }
</script>
@endsection
