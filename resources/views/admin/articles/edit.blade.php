...
<div class="article-compose-page" style="padding-top: 0;">
    ...
    @php
        $articleTagsValue = old('tags');

        if (is_array($articleTagsValue)) {
            $articleTagsValue = implode(', ', $articleTagsValue);
        }

        if ($articleTagsValue === null) {
            $articleTagsValue = is_array($article->tags)
                ? implode(', ', $article->tags)
                : (string) ($article->tags ?? '');
        }
    @endphp

    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" id="articleForm" data-initial-tags="{{ $articleTagsValue }}">
        @csrf
        @method('PUT')
        ...
        <div class="article-compose-grid">
            <section class="article-editor-card">
                <div class="article-field">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" required maxlength="120" placeholder="Nhập tiêu đề bài viết...">
                    <div class="article-counter"><span id="titleCount">0</span>/120</div>
                </div>

                <div class="article-field">
                    <label for="summary">Tóm tắt ngắn</label>
                    <textarea name="summary" id="summary" maxlength="200" rows="4" placeholder="Nhập tóm tắt ngắn gọn về nội dung bài viết (không bắt buộc)...">{{ old('summary', $article->summary) }}</textarea>
            ...
            <aside class="article-side-panel">
                <section class="article-side-card">
                    ...
                    <label class="article-toggle-row" for="is_published">
                        ...
                        <input type="checkbox" name="is_published" id="is_published" value="1" @checked(old('is_published', $article->is_published))>
                    </label>
                    ...
                    <div class="article-publish-actions">
                        ...
                        <button type="submit" class="article-btn primary" id="publishButton">Cập nhật</button>
                    </div>
                </section>
                ...
                <section class="article-side-card">
                    ...
                    <div class="article-upload-zone" id="uploadZone">
                        <div id="image-preview-placeholder" @if($article->featured_image) hidden @endif>
                        ...
                        <div id="image-preview-container" @if(!$article->featured_image) hidden @endif>
                            <img id="image-preview" src="{{ $article->featured_image ? Storage::url($article->featured_image) : '#' }}" alt="Ảnh minh họa">
                ...
                <section class="article-side-card">
                    ...
                    <div class="article-field compact">
                        ...
                        <input type="hidden" name="tags" id="tags" value="{{ $articleTagsValue }}">
...
<script>
const initialTags = document.getElementById('articleForm')?.dataset.initialTags || '';
...
function updatePublishLabel() {
    ...
    label.textContent = isPublished ? 'Đã xuất bản' : 'Đang lưu nháp';
