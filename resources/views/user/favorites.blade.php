                ...
                @if($favHerbs->count() > 0)
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 items-stretch">
                        @foreach($favHerbs as $herb)
                            <div class="bg-white/90 backdrop-blur-md rounded-[2rem] border border-slate-100/80 p-6 shadow-sm hover:shadow-[0_15px_30px_rgba(59,130,246,0.1)] hover:border-sky-200/80 transition-all flex flex-col justify-between relative overflow-hidden min-h-[300px]">
                                {{-- Compact image at the top --}}
                                <div class="w-full h-36 rounded-2xl bg-sky-50/70 p-2 flex items-center justify-center border border-slate-100/50 overflow-hidden shadow-inner mb-5 shrink-0">
                                ...
                                <div class="flex-grow flex flex-col justify-between">
                                    <div>
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wide bg-sky-50 text-sky-600 border border-sky-100/50">
                                            dược liệu
                                        </span>
                                        <h4 class="font-black text-slate-800 text-lg mt-2 leading-snug truncate">
                                            {{ $herb->name }}
                                        </h4>
                                        <p class="text-xs font-bold text-slate-400 italic truncate mt-1">
                                    ...
                                    <div>
                                        {{-- Divider --}}
                                        <div class="border-t border-slate-100/60 my-3"></div>

                                        {{-- Action Row --}}
                                        <div class="flex items-center justify-between gap-3">
                                            <a href="{{ route('herb-dictionary.show', $herb->slug) }}" class="inline-flex items-center justify-center gap-1.5 flex-grow py-2.5 bg-sky-500 hover:bg-sky-600 text-white font-extrabold rounded-full text-sm shadow-md transition-all cursor-pointer">
                                                Xem chi tiết <span class="text-[10px]">▶</span>
                                            </a>

                                            {{-- Unfavorite heart --}}
                                            <form action="{{ route('herb-dictionary.favorite', $herb->slug) }}" method="POST" class="inline shrink-0">
                                                @csrf
                                                <button type="submit" class="w-10 h-10 rounded-full bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-200 flex items-center justify-center shadow-sm text-rose-500 hover:scale-105 active:scale-95 transition-all cursor-pointer focus:outline-none" title="Bỏ thích">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 fill-current text-rose-500" viewBox="0 0 24 24">
                        ...
                        @if($favHerbs->count() < 4)
                            <div class="border-2 border-dashed border-sky-300 bg-sky-50/40 backdrop-blur-sm rounded-[2rem] p-6 flex flex-col items-center justify-center text-center min-h-[300px] w-full shadow-sm hover:bg-sky-50/60 transition-colors">
                                <svg class="w-24 h-16 text-sky-400 mb-3" viewBox="0 0 100 60" fill="none" stroke="currentColor" stroke-width="1.5">
                                ...
                                <h4 class="font-extrabold text-slate-800 text-sm mb-1.5">Còn nhiều điều thú vị đang chờ bạn!</h4>
                                <p class="text-slate-500 text-xs leading-relaxed max-w-[220px] mx-auto">
                                    Lưu thêm dược liệu hoặc khám phá các bài viết hữu ích khác để bộ sưu tập của bạn phong phú hơn nhé.
                                </p>
                                <a href="{{ route('herb-dictionary.index') }}" class="mt-5 inline-flex items-center gap-1.5 px-5 py-2.5 bg-sky-500 hover:bg-sky-600 text-white font-extrabold rounded-full text-sm shadow-md hover:scale-102 transition-all cursor-pointer">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 items-stretch">
                            ...
                            <div class="bg-white/90 backdrop-blur-md rounded-[2rem] border border-slate-100/80 p-6 shadow-sm hover:shadow-[0_15px_30px_rgba(59,130,246,0.1)] hover:border-sky-200/80 transition-all flex flex-col justify-between relative overflow-hidden min-h-[300px]">
                                ...
                                <div class="flex-grow flex flex-col justify-between">
                                    <div>
                                        ...
                                        <h4 class="font-black text-slate-800 text-lg mt-2 leading-snug line-clamp-2">
                                            {{ $article->title }}
                                        </h4>
                                    ...
                                    <div>
                                        ...
                                        {{-- Action Row --}}
                                        <div class="flex items-center justify-between gap-3">
                                            <a href="{{ route('articles.show', $article->slug) }}" class="inline-flex items-center justify-center gap-1.5 flex-grow py-2.5 bg-sky-500 hover:bg-sky-600 text-white font-extrabold rounded-full text-sm shadow-md transition-all cursor-pointer">
                                                Xem chi tiết <span class="text-[10px]">▶</span>
                                            </a>
                        ...
                        @if($favArticles->count() < 4)
                            <div class="border-2 border-dashed border-sky-300 bg-sky-50/40 backdrop-blur-sm rounded-[2rem] p-6 flex flex-col items-center justify-center text-center min-h-[300px] w-full shadow-sm hover:bg-sky-50/60 transition-colors">
                                ...
                                <a href="{{ route('articles.index') }}" class="mt-5 inline-flex items-center gap-1.5 px-5 py-2.5 bg-sky-500 hover:bg-sky-600 text-white font-extrabold rounded-full text-sm shadow-md hover:scale-102 transition-all cursor-pointer">
